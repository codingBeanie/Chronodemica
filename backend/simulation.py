...
def create_voting_entry(pop_data: Dict[str, Any], party_data: Dict[str, Any], party_info: Dict[str, Any]) -> Dict[str, Any]:
    """Create a single voting behavior entry."""
    distance = calculate_distance(
        pop_data["social_orientation"], pop_data["economic_orientation"],
        party_data["social_orientation"], party_data["economic_orientation"]
    )
    
    raw_score = calculate_score(
        pop_data["max_political_distance"], 
        pop_data["variety_tolerance"], 
        distance
    )
    
    adjusted_score = calculate_adjusted_score(
        party_data.get("political_strength", 0), 
        raw_score
    )
...
def add_special_parties_voting(pop_data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for party_id, config in SPECIAL_PARTIES_CONFIG.items():
        distance_key = 'non_voters_distance' if party_id == SPECIAL_PARTY_IDS['NON_VOTERS'] else 'small_party_distance'
...
def calculate_vote_percentages_and_votes(voting_behavior: List[Dict[str, Any]], eligible_population: int) -> List[Dict[str, Any]]:
    ...
    for entry in voting_behavior:
        percentage = (entry["adjusted_score"] / total_score * 100) if total_score > 0 else 0
...
def create_pop_votes(db: Session, period_id: int) -> None:
    ...
    for pop_period in pop_periods:
        pop_period_dict = convert_sqlmodel_to_dict(
            pop_period,
            ["pop_id", "period_id", "social_orientation", "economic_orientation",
             "max_political_distance", "variety_tolerance", "non_voters_distance",
             "small_party_distance", "ratio_eligible", "pop_size"]
        )
...
def calculate_election_result_data(party_id: int, party_votes: int, sum_votes: int, threshold: float) -> Dict[str, Any]:
...
def upsert_election_result(db: Session, result_data: Dict[str, Any]) -> None:
    ...
    existing_entries = get_items(
        db, ElectionResult,
        filters={"period_id": result_data["period_id"], "party_id": result_data["party_id"]}
    )
...
def create_election_results(db: Session, period_id: int, seats: int, threshold: float) -> None:
    ...
    for party_id, party_votes in party_votes_summary.items():
        result_data = calculate_election_result_data(party_id, party_votes, sum_votes, threshold)
...
def allocate_residual_seats(parliament_data: List[Dict[str, Any]], seats_left: int) -> None:
...
def calculate_seats(db: Session, period_id: int, seats: int) -> None:
    ...
    if not parliament:
        raise HTTPException(
            status_code=404, 
            detail="No parties in parliament for the selected period"
        )
...
def gather_simulation_statistics(db: Session, period_id: int) -> Dict[str, Any]:
    ...
    total_parties = len([r for r in election_results if r.party_id > 0])  # Exclude special parties
...
def run_complete_simulation(db: Session, period_id: int, seats: int, threshold: float) -> Dict[str, Any]:
    """
...
def get_party_details_and_orientations(db: Session, period_id: int, parties_with_seats: List) -> tuple[Dict, Dict]:
...
def create_coalition_data(combination, party_details: Dict, party_orientations: Dict, coalition_id: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "majority_margin": coalition_seats - (sum(party.seats for party in combination for party in [combination[0]]) // 2 + 1) + 1,  # This needs fixing
...
def getCoalitions(db: Session, period_id: int) -> List[Dict[str, Any]]:
    ...
    party_details, party_orientations = get_party_details_and_orientations(db, period_id, parties_with_seats)
    ...
    for size in range(1, len(parties_with_seats) + 1):
        for combination in combinations(parties_with_seats, size):
            ...
            if coalition_seats >= majority_threshold:
                ...
                is_superset = any(
                    minimal_set.issubset(current_party_ids) and minimal_set != current_party_ids
                    for minimal_set in minimal_coalition_party_sets
                )
                
                # Only add if it's not a superset of an existing minimal coalition
                if not is_superset:
                    coalition = create_coalition_data(combination, party_details, party_orientations, len(coalitions) + 1)
                    coalition["majority_margin"] = coalition_seats - majority_threshold + 1  # Fix the calculation
